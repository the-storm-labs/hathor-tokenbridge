...
type ReadCallFn = 'isProcessed' | 'isSigned' | 'isProposed' | 'getSignatureCount';
type ReadCall = (fn: ReadCallFn, callArgs: any[]) => Promise<any>;

// Bundles everything a single transactionId's replay needs, so per-transaction helpers don't
// have to take a dozen positional parameters each.
interface ReplayContext {
  config: ConfigData;
  hathorFederationContract: IHathorFederationV1;
  rawContract: any;
  pinBlock: number | undefined;
  maxSignaturesEnv: string;
  logger: LogWrapper;
  readCall: ReadCall;
  brokers?: { evmBroker: EvmBroker; hathorBroker: HathorBroker };
}
...
// Read either via the typed wrapper (live) or via the raw contract pinned to pinBlock.
function buildReadCall(
  hathorFederationContract: IHathorFederationV1,
  rawContract: any,
  pinBlock: number | undefined,
  logger: LogWrapper,
  ...
  return async (fn, callArgs) => {
    if (pinBlock === undefined) {
      return (hathorFederationContract as any)[fn](...callArgs);
    }
    try {
      return await rawContract.methods[fn](...callArgs).call({}, pinBlock);
    } catch (err) {
      logger.warn(
        `Pinned read of ${fn} @block ${pinBlock} failed (likely non-archive RPC), falling back to live: ${err}`,
      );
      return (hathorFederationContract as any)[fn](...callArgs);
    }
  };
...
async function buildValidationBrokers(
  config: ConfigData,
  logger: LogWrapper,
): Promise<{ evmBroker: EvmBroker; hathorBroker: HathorBroker }> {
  ...
  const evmBroker = new EvmBroker(config, logger, bridgeFactory, federationFactory, metricRegister);
...
async function collectSignatures(
  transactionId: string,
  ctx: ReplayContext,
): Promise<{ signatures: string[]; slice: string[] }> {
  const { hathorFederationContract, rawContract, pinBlock, maxSignaturesEnv, readCall } = ctx;
  const freshCount = await readCall('getSignatureCount', [transactionId]);
  const signatures: string[] = [];
  for (let i = 0; i < freshCount; i++) {
    const sig =
      pinBlock === undefined
        ? await hathorFederationContract.transactionSignatures(transactionId, i)
        : await rawContract.methods.transactionSignatures(transactionId, i).call({}, pinBlock);
    signatures.push(sig as unknown as string);
  }
  const slice = signatures.slice(0, Number.parseInt(maxSignaturesEnv));
  return { signatures, slice };
}

// Cross-check using the SAME logic pushProposal now uses (selectCompleteSignatures,
// src/lib/utils.ts): does each of the naive slice(0,maxSignatures) signers actually cover
// every input index the decoded tx requires? A signer missing coverage for one of the tx's
// inputs is exactly the shape of error that manifests on Hathor's side as "Signatures are
// incompatible with redeemScript".
function logSignatureCoverage(signatures: string[], slice: string[], inputCount: number, ctx: ReplayContext): void {
  const { logger, maxSignaturesEnv } = ctx;
  ...
  const fixedSelection = selectCompleteSignatures(signatures, inputCount).slice(0, Number.parseInt(maxSignaturesEnv));
  logger.info(
  ...
  if (fixedSelection.length < Number.parseInt(maxSignaturesEnv)) {
    logger.warn(
...
async function runValidateTxDiagnostics(
  info: EventInfo,
  txHex: string,
  signatures: string[],
  slice: string[],
  ...
  const { config, logger, brokers } = ctx;

  // Raw wallet/decode call first — same endpoint Broker#decodeTxHex uses (read-only, never
  // pushes), but logged here with the FULL response body instead of decodeTxHex's own
  // `${response.status} - ${response.data}` (which stringifies an object to "[object Object]").
  try {
    ...
    if (decodeResponse.data?.success && typeof inputCount === 'number') {
      logSignatureCoverage(signatures, slice, inputCount, ctx);
    }
  } catch (err) {
    logger.error(`wallet/decode call itself failed: ${err}`);
  }

  const broker: Broker = info.transactionType === TransactionTypes.MELT ? brokers!.hathorBroker : brokers!.evmBroker;
  try {
...
// ---- STEP 2: replicate pushProposal's decision logic for one transactionId (all safe reads) ----
async function replayTransaction(info: EventInfo, ctx: ReplayContext): Promise<void> {
  const { readCall, logger, maxSignaturesEnv, hathorFederationContract, brokers } = ctx;
  ...
  const { signatures, slice } = await collectSignatures(info.transactionId, ctx);
  ...
  if (brokers) {
    await runValidateTxDiagnostics(info, txHex, signatures, slice, ctx);
  }
}
...
async function main() {
  ...
  const byTxId = await scanEvents(hathorFederationContract, FROM_BLOCK, TO_BLOCK, BATCH_SIZE, logger);
  const readCall = buildReadCall(hathorFederationContract, rawContract, PIN_BLOCK, logger);
  const brokers = RUN_VALIDATE_TX ? await buildValidationBrokers(config, logger) : undefined;
  ...
  for (const info of byTxId.values()) {
    await replayTransaction(info, ctx);
  }
