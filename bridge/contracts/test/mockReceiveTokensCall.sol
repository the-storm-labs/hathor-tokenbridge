...
contract mockReceiveTokensCall is IERC777Recipient {
    ...
    function callReceiveTokens(address tokenToUse, string memory receiver, uint256 amount, uint256 destinationChainId) external {
        IERC20(tokenToUse).approve(bridge, amount);
        IBridge(bridge).receiveTokensTo(destinationChainId, tokenToUse, receiver, amount);
    }

    function callDepositTo(string memory receiver, uint256 destinationChainId) external payable {
